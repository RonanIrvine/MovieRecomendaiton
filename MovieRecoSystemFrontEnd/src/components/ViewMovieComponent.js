...
const ViewMovieComponet=()=>{
    ...
    console.log("Type:"+usertype+" Name: "+username);
    //list component
    ...
    const handleChange = event => {
      ...
      const getMovieRating=(id)=> {
         RatingsSerive.getMovieRating(id) .then((response)=>{//call function to pass id to API n retrieve the movie information
    ...
    //  console.log(favmovie.title);
     UserFavService.addmovietofav(favmovie) .then((response) => {//call API to pass fav movie data
      if (response.status === 200) {//if success 
        console.log(response.data);
        alert("Movie Addded to favourites"); //navigate page
      } else {//else
